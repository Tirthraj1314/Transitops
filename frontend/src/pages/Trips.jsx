...
const SEARCH_FIELDS = ["source", "destination", "vehicle.registrationNumber", "driver.name", "status"];
...
function TripsManager() {
  ...
  const { search, setSearch, sortKey, sortDir, toggleSort, page, setPage, totalPages, totalCount, rows } =
    useTableControls(trips, { searchFields: SEARCH_FIELDS });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="max-w-xs">
        <TableSearch value={search} onChange={setSearch} placeholder="Search trips..." />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="border-b bg-gray-50 text-gray-500 dark:border-slate-800 dark:bg-slate-800/60 dark:text-slate-400">
              <tr>
                <SortableHeader label="Route" sortKey="source" activeKey={sortKey} activeDir={sortDir} onSort={toggleSort} />
                <SortableHeader label="Vehicle" sortKey="vehicle.registrationNumber" activeKey={sortKey} activeDir={sortDir} onSort={toggleSort} />
                <SortableHeader label="Driver" sortKey="driver.name" activeKey={sortKey} activeDir={sortDir} onSort={toggleSort} />
                <SortableHeader label="Status" sortKey="status" activeKey={sortKey} activeDir={sortDir} onSort={toggleSort} />
