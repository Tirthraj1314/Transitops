...
export default function Maintenance() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-lg font-semibold text-gray-800">Maintenance</h1>
            ...
            {records.length === 0 ? (
              ...
              records.map((record) => (
                <tr key={record.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium text-gray-800">{record.vehicle}</td>
                  <td className="px-4 py-3 text-gray-600">{record.serviceType}</td>
                  <td className="px-4 py-3 text-gray-600">{record.scheduledDate}</td>
                  <td className="px-4 py-3 text-gray-600">₹{record.cost?.toLocaleString()}</td>
